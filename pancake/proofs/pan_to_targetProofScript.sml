...
word_depthProofTheory.max_depth_call_graph




Theorem pan_lang_safe_imp_word_lang_safe:
  wprog0 = pan_to_word_compile_prog prog ∧
  s.code = fromAList (SND (word_to_word_compile c.word_to_word_conf mc.target.config wprog0)) ⇒
  pan_lang_is_safe_for_space c prog (read_limits c (mc:(α,β,γ) machine_config) ms) ⇒
  word_lang_safe_for_space s start   
Proof
  simp[wordSemTheory.word_lang_safe_for_space_def]>>
  rpt strip_tac>>
  gs[pan_lang_is_safe_for_space_def]>>
  strip_tac>>
  cheat
QED
...
Theorem pan_to_target_compile_semantics':
  ...
  s.ffi = ffi ∧ mc.target.config.big_endian = s.be ∧
  pan_installed bytes cbspace bitmaps data_sp c'.lab_conf.ffi_names (heap_regs c.stack_conf.reg_names)
                mc ms (mk_mem (make_funcs (compile_prog pan_code)) s.memory) s.memaddrs ∧ X = 
  semantics s start ≠ Fail ⇒
  machine_sem (mc:(α,β,γ) machine_config) (ffi:'ffi ffi_state) ms ⊆
              extend_with_resource_limit' (pan_lang_is_safe_for_space c.lab_conf.asm_conf pan_code (read_limits c mc ms)) {semantics (s:('a,'ffi) panSem$state) start}
Proof
  strip_tac>>
  last_x_assum mp_tac>>
  rewrite_tac[pan_to_targetTheory.compile_prog_def]>>
...
(* here *)
  ...
  gs[]>>
  rpt strip_tac
  >- (irule (INST_TYPE [] pan_lang_safe_imp_word_lang_safe)>>
      pop_assum mp_tac>>
      qpat_assum ‘c.lab_conf.asm_conf = _’ (fn h => ONCE_REWRITE_TAC[GSYM h])>>
      strip_tac>>
      pop_assum $ irule_at Any>>
      gs[])>>
  gs[semanticsPropsTheory.extend_with_resource_limit_def]>>
...
Theorem pan_to_target_compile_semantics':
  ...
  semantics s start ≠ Fail ⇒
  machine_sem (mc:(α,β,γ) machine_config) (ffi:'ffi ffi_state) ms ⊆
              extend_with_resource_limit' (option_le stack_max (SOME (FST (read_limits c mc ms)))) {semantics (s:('a,'ffi) panSem$state) start}
Proof
  strip_tac>>
  last_x_assum mp_tac>>
...
(* here *)
  ...
  once_rewrite_tac[GSYM ADD_DIV_ADD_DIV]





  qpat_x_assum ‘w2n _ DIV _ - LENGTH _ = w2n _ DIV _’ assume_tac>>
  simp[word_add_def]>>
