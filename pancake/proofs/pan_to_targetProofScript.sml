...
(* current *)
Theorem memory_fun2set_IMP_read:
...
qmatch_asmsub_abbrev_tac ‘memory sss.memory (addresses _ heaplen) * _’>>
   ‘∃r. (memory sss.memory (addresses s.base_addr heaplen) * r) (fun2set (m, t.mem_domain))’ by cheat>>
   
   drule memory_fun2set_IMP_read>>strip_tac>>

(*       
   qpat_x_assum ‘(_ * _) (fun2set (sss.memory,sss.mdomain))’ mp_tac>>
   simp[GSYM set_sepTheory.STAR_ASSOC]>>strip_tac>>
...
Theorem fun2set_update_eq[simp]:
  fun2set (m, md) = fun2set (m', md') ∧ x ∈ md ∧ x ∈ md' ⇒
  fun2set (m⦇x ↦ a⦈, md) = fun2set (m'⦇x ↦ a⦈, md')
Proof
  rw[set_sepTheory.fun2set_def]>>gs[EXTENSION,EQ_IMP_THM]>>
  rw[UPDATE_def]>>IF_CASES_TAC>>gs[]
  >- (first_x_assum $ qspec_then ‘(a', m a')’ assume_tac>>gs[])>>
  first_x_assum $ qspec_then ‘(a', m' a')’ assume_tac>>gs[]
QED        
...
Theorem mem_load_const_memory[simp]:
  fun2set (m,s.mdomain) = fun2set (s.memory,s.mdomain) ⇒
   wordSem$mem_load ad (s with memory := m) = mem_load ad s
Proof
  strip_tac>>gs[wordSemTheory.mem_load_def]>>
  CASE_TAC>>gs[set_sepTheory.fun2set_def,EXTENSION,EQ_IMP_THM]>>
  first_x_assum $ qspec_then ‘(ad, m ad)’ assume_tac>>gs[]
QED

Theorem mem_store_const_memory[simp]:
  fun2set (m,s.mdomain) = fun2set (s.memory,s.mdomain) ⇒
  (mem_store ad w s = NONE ⇔ wordSem$mem_store ad w (s with memory := m) = NONE) ∧
  (∀x. mem_store ad w s = SOME x ⇒
       wordSem$mem_store ad w (s with memory := m) = SOME (s with memory := m⦇ad↦w⦈))
Proof
  strip_tac>>gs[wordSemTheory.mem_store_def]>>metis_tac[]
QED
...
Theorem mem_load_byte_aux_const_memory:
     fun2set (m,dm) = fun2set (m',dm) ⇒
     wordSem$mem_load_byte_aux m' dm be w = mem_load_byte_aux m dm be w 
Proof
  strip_tac>>gs[wordSemTheory.mem_load_byte_aux_def]>>
  gs[EXTENSION,set_sepTheory.fun2set_def]>>
  pop_assum $ qspec_then ‘(byte_align w, m (byte_align w))’ assume_tac>>
  gs[]>>
  rpt (CASE_TAC>>gs[])
QED
...
Theorem mem_store_byte_aux_const_memory:
     fun2set (m,dm) = fun2set (m',dm) ⇒
     (mem_store_byte_aux m dm be w b = NONE ⇔ mem_store_byte_aux m' dm be w b = NONE) ∧
...
Proof
  ...
  Cases_on ‘byte_align w ∈ dm’>>gs[]
  >- (‘m' (byte_align w) = m (byte_align w)’
        by (gs[EXTENSION]>>
            first_x_assum $ qspec_then ‘(byte_align w, m (byte_align w))’ assume_tac>>
            gs[])>>gs[]>>
      CASE_TAC>>
      gs[GSYM set_sepTheory.fun2set_def]>>
      srw_tac[ETA_ss][])>>
     CASE_TAC>>CASE_TAC
QED
...
Theorem inst_const_memory:
  ...
  (∀x. inst i s = SOME x ⇒
       (∃m'. THE (wordSem$inst i (s with memory := m)) = x with memory := m' ∧
...
Proof
  ...
  >- (gs[wordSemTheory.assign_def, wordSemTheory.set_var_def]>>
      CASE_TAC>>gs[word_exp_const_memory]>>metis_tac[])
  >- (rpt (CASE_TAC>>
      gs[wordSemTheory.assign_def,word_exp_const_memory,
         wordSemTheory.set_var_def,get_vars_const_memory])>>
      metis_tac[])
  >- (rpt (CASE_TAC>>gs[])>>
      ...
      imp_res_tac mem_load_byte_aux_const_memory>>gs[]>>
      first_x_assum $ qspecl_then [‘c''’, ‘s.be’, ‘w2w c’] assume_tac>>
      gs[]>>TRY (metis_tac[])>>
      gvs[wordSemTheory.mem_store_def]>>
...
Theorem memory_swap_lemma:
  ∀prog st res rst m.
     wordSem$evaluate (prog, st) = (res, rst) ∧
     (∀a b c. st.gc_fun (a, st.memory, b, c) = st.gc_fun (a, m, b, c))  ∧
...
Proof
  recInduct wordSemTheory.evaluate_ind >>srw_tac[][wordSemTheory.evaluate_def]>>full_simp_tac(srw_ss())[wordSemTheory.call_env_def]
  >- metis_tac[]
  >- (CASE_TAC>>gs[wordSemTheory.get_var_def]
      >- metis_tac[]>>
      CASE_TAC>>gs[]
      >- ( (* alloc *)
       gs[wordSemTheory.alloc_def]>>
       rpt (CASE_TAC>>gvs[])
       >- metis_tac[]
       >- (FULL_CASE_TAC>>gvs[]
           >- metis_tac[]>>
           gs[wordSemTheory.set_store_def,
              wordSemTheory.push_env_def]>>
           pairarg_tac>>gs[wordSemTheory.gc_def])
       >- (FULL_CASE_TAC>>gvs[wordSemTheory.flush_state_def]>>
           gs[wordSemTheory.set_store_def,
              wordSemTheory.push_env_def]>>
           pairarg_tac>>gs[wordSemTheory.gc_def]>>
           rpt (FULL_CASE_TAC>>gvs[wordSemTheory.pop_env_def])>>
           gs[wordSemTheory.dec_stack_def,wordSemTheory.enc_stack_def]>>
           FULL_CASE_TAC>>gs[])
       >- (FULL_CASE_TAC>>gvs[wordSemTheory.flush_state_def]>>
           gs[wordSemTheory.set_store_def,
              wordSemTheory.push_env_def]>>
       ...
       >- (FULL_CASE_TAC>>gvs[wordSemTheory.flush_state_def]>>
           ...
           gs[wordSemTheory.dec_stack_def,wordSemTheory.enc_stack_def]>>
           FULL_CASE_TAC>>gs[]>>metis_tac[])
       >- (FULL_CASE_TAC>>gvs[wordSemTheory.set_store_def,
                              wordSemTheory.push_env_def]>>
           pairarg_tac>>gs[wordSemTheory.gc_def]>>
           rpt (FULL_CASE_TAC>>gvs[wordSemTheory.pop_env_def])>>
           gs[wordSemTheory.dec_stack_def,wordSemTheory.enc_stack_def]>>
...
(* not sure this actually works *)
       ...
       first_x_assum $ qspec_then ‘m'’ assume_tac>>gs[]>>
       pop_assum $ irule>>gs[]>>
       drule wordPropsTheory.evaluate_consts>>strip_tac>>gs[]>>cheat )>>
      gvs[]>>
      first_x_assum $ qspec_then ‘m’ assume_tac>>gs[]>>metis_tac[])
  ...
  Cases_on ‘find_code dest (add_ret_loc ret x) s.code s.stack_size’>>gs[]>- metis_tac[]>>
  rename1 ‘_ = SOME x'’>>PairCases_on ‘x'’>>gs[]>>
  Cases_on ‘ret’>>gs[]
  >- (IF_CASES_TAC>>gs[]
      >- (IF_CASES_TAC>>gs[wordSemTheory.flush_state_def]>>gvs[]>-metis_tac[]>>
  ...
  >- (IF_CASES_TAC>>gvs[wordSemTheory.flush_state_def,wordSemTheory.push_env_def]>>
      ...
      Cases_on ‘evaluate (x1, s1)’>>rename1 ‘evaluate _ = (q4, r)’>>gs[]>>
      Cases_on ‘∃y. q4 = SOME (Result (Loc q''' r') y)’>>gs[]
      >- (Cases_on ‘pop_env r’>>gs[]
          >- (first_x_assum $ qspec_then ‘m’ assume_tac>>gs[wordSemTheory.pop_env_def]>>
              rpt (CASE_TAC>>gs[])>>metis_tac[])>>
          rename1 ‘domain x''.locals = domain x'’>>
          Cases_on ‘domain x''.locals = domain x'’>>gs[wordSemTheory.set_var_def]
          >- (last_x_assum $ qspec_then ‘m’ assume_tac>>gs[wordSemTheory.pop_env_def]>>
              rpt (CASE_TAC>>gs[])>>gvs[]>>
              last_x_assum $ qspec_then ‘m''’ assume_tac>>gs[]>>
              ‘(∀a b c. r.gc_fun (a,r.memory,b,c) = r.gc_fun (a,m'',b,c))’ by cheat>>gs[]>>
              metis_tac[])>>
          first_x_assum $ qspec_then ‘m’ assume_tac>>gs[wordSemTheory.pop_env_def]>>
          rpt (CASE_TAC>>gs[])>>gvs[]>>metis_tac[])>>
      first_x_assum $ qspec_then ‘m’ assume_tac>>gs[]>>
      Cases_on ‘q4’>>gs[]>-metis_tac[]LL
      rename1 ‘evaluate _ = (SOME x2, r)’>>Cases_on ‘x2’>>gs[]>>metis_tac[])
  ...
  >- (IF_CASES_TAC>>gvs[wordSemTheory.flush_state_def,wordSemTheory.push_env_def]>>
      ...
      rename1 ‘evaluate _ = (SOME x2, r)’>>Cases_on ‘x2’>>gs[]>>metis_tac[])(* handler = NONE *)
  >>
  qmatch_goalsub_abbrev_tac ‘push_env x1 (SOME x2)’>>
  PairCases_on ‘x2’>>gs[wordSemTheory.push_env_def]>>
  pairarg_tac>>gs[wordSemTheory.flush_state_def]>>
  IF_CASES_TAC>>gvs[wordSemTheory.dec_clock_def]>- metis_tac[]>>
  ...
  Cases_on ‘∃x0 y0. x2 = Exception x0 y0’>>gs[]>>
  rpt (CASE_TAC>>gs[wordSemTheory.set_var_def])
  >-metis_tac[]
  >- (first_x_assum $ qspec_then ‘m''’ assume_tac>>gs[]>>
      ‘(∀a b c. r.gc_fun (a,r.memory,b,c) = r.gc_fun (a,m'',b,c))’ by cheat>>gs[]>>
      metis_tac[]>>metis_tac[])>>
...
Theorem word_semantics_memory_update':
  (∀a b c. s.gc_fun (a,s.memory,b,c) = s.gc_fun (a,m,b,c)) ∧
  fun2set (s.memory,s.mdomain) = fun2set (m,s.mdomain) ⇒
...
Proof
  ...
  >- (strip_tac>>gs[]>>
      ...
      >- (Cases_on ‘r=SOME TimeOut’>>gs[]>>
          ‘FST
           (evaluate
            (Call NONE (SOME start) [0] NONE,
             s with <|memory := m; clock := k'|>)) = r’ by cheat>>
          gs[]>>Cases_on ‘r’>>rename1 ‘SOME x'’>>Cases_on ‘x'’>>gs[]>>
          first_x_assum $ qspec_then ‘k’ assume_tac>>gs[])>>
      DEEP_INTRO_TAC some_intro >> simp[] >>
      strip_tac
      >- cheat >>
      qexists_tac ‘k’>>gs[]>>metis_tac[])>>
  IF_CASES_TAC>>gs[]
  >- (last_x_assum $ qspec_then ‘k’ assume_tac>>gs[]>>
      last_x_assum $ qspec_then ‘k’ assume_tac>>gs[]>>
      qmatch_asmsub_abbrev_tac ‘ev = _ ⇒ _’>>
...
Proof
  ...
  >- (strip_tac>>gs[]>>
      ...
      >- (strip_tac>>
          ...
          strip_tac>>gs[]>>
          ‘q = r''’ by cheat>>gs[]>>
          ‘t'.ffi.io_events = r'.ffi.io_events’ by cheat>>gs[]>>
...
(*  disch_then $ drule_at Any>>
  ...
  (* memory shift *)
  ‘fun2set (wst0.memory, wst0.mdomain) = fun2set (mk_mem (make_funcs (compile_prog pan_code)) s.memory,wst0.mdomain)’ by cheat>>
  drule_at Any word_semantics_memory_update>>
  ‘∀a b c. wst0.gc_fun (a,wst0.memory,b,c) =
              wst0.gc_fun
                (a,mk_mem (make_funcs (compile_prog pan_code)) s.memory,b,c)’ by cheat>>
  disch_then $ drule_at Any>>
  disch_then $ qspec_then ‘InitGlobals_location’ mp_tac>>
  strip_tac>>
  ‘semantics (wst0 with memory := mk_mem (make_funcs (compile_prog pan_code)) s.memory) InitGlobals_location ≠ Fail ⇒
   semantics wst0 InitGlobals_location ≠ Fail’
    by (rw[]>>gs[])>>
  pop_assum $ irule_at Any>>
  ...
  gs[GSYM CONJ_ASSOC]>>
(*
  ‘fun2set (m, s.memaddrs) = fun2set (sst.memory, sst.mdomain) ∧
   semantics wst0 InitGlobals_location ≠ Fail ⇒
   semantics wst0 InitGlobals_location =
   semantics (wst0 with memory := mk_mem (make_funcs (compile_prog pan_code)) s.memory) InitGlobals_location’
    by (strip_tac>>irule word_semantics_memory_update>>gs[]>>
        gs[Abbr ‘wst0’, Abbr ‘wst’]>>
        gs[word_to_stackProofTheory.make_init_def]>>
        conj_tac>-cheat>>
        qpat_x_assum ‘fun2set _ = _’ $ assume_tac o GSYM>>
        gs[]>>pop_assum kall_tac>>
        simp[set_sepTheory.fun2set_def]>>
        simp[EXTENSION]>>strip_tac>>
        simp[EQ_IMP_THM]>>strip_tac>>strip_tac
        >- (qpat_x_assum ‘∀a. _ ⇒ m _ = mk_mem _ _ _’ assume_tac>>
            first_assum $ qspec_then ‘a’ assume_tac>>fs[]>>
            (* a ∈ sst.mdomain *)
            gs[stack_to_labProofTheory.full_make_init_def]>>
            gs[stack_removeProofTheory.make_init_opt_def,
               stack_allocProofTheory.make_init_def,
               ...
               stack_to_labProofTheory.make_init_def,
               stack_removeProofTheory.make_init_any_def]>>
            Cases_on ‘opt’>>gs[stack_removeProofTheory.init_reduce_def]>>
            gvs[]>>FULL_CASE_TAC>>gs[]>>
            qmatch_asmsub_abbrev_tac ‘evaluate (init_code gcf _ _, s0)’>>
            qabbrev_tac ‘ev = evaluate (init_code gcf max_heap sp, s0)’>>
            Cases_on ‘ev’>>gs[]>>rename1 ‘(q',r')’>>Cases_on ‘q'’>>gs[]>>
            gs[targetSemTheory.good_init_state_def]>>
            gs[targetSemTheory.good_init_state_def]>>
            gs[targetSemTheory.good_init_state_def]>>
            gvs[stack_removeProofTheory.init_prop_def]


  conj_asm1_tac>- cheat>>
(*   (gs[Abbr ‘wst0’, Abbr ‘wst’]>>
    simp[word_to_stackProofTheory.make_init_def])*)
*)
...
(*
  irule_at Any (INST_TYPE [beta|-> “:num # α lab_to_target$config”] pan_to_wordProofTheory.state_rel_imp_semantics)>>gs[]>>
  gs[Abbr ‘m0’]>>irule_at Any EQ_REFL>>gs[]>>
*)
