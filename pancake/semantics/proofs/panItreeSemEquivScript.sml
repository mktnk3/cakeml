...
Proof
  ...
  >- (rw [Once evaluate_def] >>
      ...
      >- (drule_then assume_tac FUNPOW_Tau_Ret_inner_eq >> gvs [] >>
          qexistsl_tac [‘0’,‘k'’] >>
          simp [empty_locals_defs] >>
          gvs [bind_FUNPOW_eqn,h_handle_call_ret_def,mrec_sem_simps,
               ltree_lift_cases] >>
      ...
      >- (drule_then assume_tac FUNPOW_Tau_Ret_inner_eq >> gvs [] >>
          ...
          gvs [bind_FUNPOW_eqn,h_handle_call_ret_def,mrec_sem_simps,
               ltree_lift_cases])
      >- (Cases_on ‘o'’ >> rw []
          >- (gvs [bind_FUNPOW_eqn,h_handle_call_ret_def,
                   mrec_sem_simps,ltree_lift_cases] >>
...
(*
    ...
    0.  good_dimindex (:α)
    1.  ∀p. ¬(ltree_lift query_oracle s.ffi
               (mrec_sem (h_prog (Call o' e l,unclock s))) ≈ Ret p)
    2.  ∀k. k' ≤ k ⇒
            LENGTH
   ...
   ------------------------------------
        LFINITE
          (stree_trace query_oracle event_filter s.ffi
             (to_stree (mrec_sem (h_prog (Call o' e l,unclock s)))))
    ...
    0.  ∀s. good_dimindex (:α) ∧
            ...
            (∃k'. ∀k.
               k' ≤ k ⇒
               LENGTH (SND (evaluate (prog,s with clock := k))).ffi.io_events =
               LENGTH
    ...
    1.  good_dimindex (:α)
    2.  ∀p. ¬(ltree_lift query_oracle s.ffi
               (mrec_sem (h_prog (While e prog,unclock s))) ≈ Ret p)
    3.  ∀k. k' ≤ k ⇒
            LENGTH
              (SND (evaluate (While e prog,s with clock := k))).ffi.io_events =
            LENGTH
              (SND (evaluate (While e prog,s with clock := k'))).ffi.
              io_events
   ------------------------------------
        LFINITE
          (stree_trace query_oracle event_filter s.ffi
             (to_stree (mrec_sem (h_prog (While e prog,unclock s)))))
...
Theorem LCONS_nonret_imp_Vis2:
  ...
  (∀p. ¬(ltree_lift q st ht ≈ Ret p)) ⇒
  ∃n e k.
    ht = FUNPOW Tau n ((Vis e k)) ∧ p (FST (q st (FST e)))
Proof
  strip_tac>>
  Cases_on ‘∃t. strip_tau ht t’>>fs[]
  >- (imp_res_tac strip_tau_FUNPOW>>fs[]>>
...
Theorem to_stree_FUNPOW_Tau_thm:
  to_stree X = FUNPOW Tau n (Vis e k) ⇒
  ∃n' e' k'. X = FUNPOW Tau n' (Vis e' k') ∧ n' = n ∧ e = FST e'
  ∧ k = to_stree o k' o (SND e')
Proof
  MAP_EVERY qid_spec_tac [‘e’,‘k’,‘X’,‘n’]>>
  Induct_on ‘n’>>rpt gen_tac>>strip_tac>>fs[]
  >- (Cases_on ‘X’>>gvs[to_stree_simps])>>
  Cases_on ‘X’>>fs[FUNPOW_SUC,to_stree_simps]
QED
...
Proof
  strip_tac>>
  simp[Once itree_bisimulation]>>
  qexists ‘CURRY {(t,spin)|(∀p. ¬(ltree_lift f st t ≈ Ret p))}’>>
  gvs[]>>rw[]
  >- (fs[ltree_lift_cases]>>
      fs[Once itree_wbisim_cases])
  >- (irule EQ_SYM>>simp[Once spin])
  >- fs[ltree_lift_cases]>>
...
QED
Theorem mrec_sem_nonret_bind:
  (∀x. ¬(mrec_sem ht ≈ Ret x))
  ⇒ mrec_sem ht >>= k = mrec_sem ht
Proof
  strip_tac >> CONV_TAC SYM_CONV >>
  rw[Once itree_bisimulation] >>
  qexists ‘CURRY {(mrec_sem ht, mrec_sem ht >>= k) |
           (∀x. ¬(mrec_sem ht ≈ Ret x))}’ >>
  conj_tac >- (rw[EXISTS_PROD] >> metis_tac[]) >>
  pop_assum kall_tac >>
  rw[] >> gvs[]
  ...
  Cases_on ‘a'’>>
  gvs[mrec_sem_simps] >>
  simp[Once (GSYM (cj 4 mrec_sem_simps))]>>
  rewrite_tac[Once (GSYM (cj 2 itree_bind_thm))]>>
  rewrite_tac[Once (GSYM (cj 4 mrec_sem_simps))]>>
  strip_tac>>
  qexists ‘Tau (g s)’>>fs[mrec_sem_simps]>>

      
QED

Theorem FUNPOW_Tau_bind_Vis_thm:
  ∀n t k e g.
  t >>= k = FUNPOW Tau n (Vis e g) ⇒
       ∃n' e' g'.
         t = FUNPOW Tau n' (Vis e' g')
Proof
  Induct>>rw[FUNPOW_SUC]>>
...
(*
  ...
  fs[]>>
  Cases_on ‘X’>>gvs[]>>
  dxrule_then drule LCONS_nonret_imp_Vis>>strip_tac>>
*)
(*  drule to_stree_FUNPOW_Tau_thm>>strip_tac>>fs[]>>
  ‘∀p. ¬ (ltree_lift query_oracle s.ffi (Vis e' k') ≈ Ret p)’
    by (irule nonret_trans>>
        irule_at Any ltree_lift_FUNPOW_wbisim>>
        metis_tac[])>>*)

(*  qpat_x_assum ‘∀p. ¬(ltree_lift _ _ (FUNPOW _ _ _)≈ _) ’ kall_tac>>*)
(*  qpat_x_assum ‘to_stree _ = _’ kall_tac>>
  Cases_on ‘e'’>>fs[ltree_lift_cases]>>
  pairarg_tac>>fs[]>>*)
...
(****)
  rpt (pop_assum mp_tac)>>
  MAP_EVERY qid_spec_tac [‘e’,‘k’,‘prog’,‘s’,‘n’]>>
...
fs[Abbr‘X’]>>
   ...
   fs[Abbr‘Y’]>>
   drule nonret_imp_spin>>
   strip_tac>>
   Cases_on ‘mrec_sem (h_prog (c,unclock s))’>>
   fs[to_stree_simps,stree_trace_simps,ltree_lift_cases]
   >- (fs[Once spin]
...
(***)
   ...
   rpt (pop_assum mp_tac)>>
   MAP_EVERY qid_spec_tac [‘e’,‘c’,‘s’,‘prog’]>>


  qpat_abbrev_tac ‘X=stree_trace _ _ _ _’>>
      CCONTR_TAC>>
      Cases_on ‘X’>>fs[]>>
      drule_all LCONS_nonret_imp_Vis>>gvs[]>>
      pop_assum kall_tac>>
      strip_tac>>gvs[]>>
...
(* q = NONE *)
      >- (drule_then drule ltree_Ret_to_evaluate'>>
          strip_tac>>fs[]>>
          imp_res_tac stree_trace_ret_events'>>gvs[]>>
          qmatch_asmsub_abbrev_tac ‘to_stree X’>>
          Cases_on ‘X’>>
          fs[mrec_sem_simps,ltree_lift_cases,to_stree_simps,
             LAMBDA_PROD]
          >- (pairarg_tac>>
              qpat_x_assum ‘Ret _ ≈ Ret _’ mp_tac>>
              simp[Once itree_wbisim_cases]>>strip_tac>>strip_tac>>fs[]>>
              fs[mrec_sem_simps,ltree_lift_cases,to_stree_simps,stree_trace_simps]>>
              Cases_on ‘n'’>>fs[FUNPOW_SUC]>>
              qpat_x_assum ‘to_stree _ = FUNPOW _ _ _’ mp_tac>>
              simp[]>>
              ‘p2 = unclock (reclock r)’ by simp[]>>
              pop_assum (fn h => rewrite_tac [h])>>
              last_x_assum irule>>
              fs[]>>
              conj_tac >- 
               (rpt strip_tac>>
                Cases_on ‘k''' < k''’
                >- (drule (GSYM LESS_ADD)>>strip_tac>>fs[]>>
                    qspecl_then [‘While e c’,‘reclock p2 with clock := k'''’,‘p’] assume_tac(panPropsTheory.evaluate_add_clock_io_events_mono)>>
                    fs[IS_PREFIX_APPEND]>>
                    ...
                    fs[empty_locals_defs,LAPPEND_NIL_2ND]>>
                    simp[SimpR“$=”,
                         Once evaluate_def,panPropsTheory.eval_upd_clock_eq,
                         panPropsTheory.opt_mmap_eval_upd_clock_eq1]>>gvs[]>>
                    ‘dec_clock (s with clock := SUC k') = s with clock := k'’
                      by simp[dec_clock_def]>>fs[]>>
                    strip_tac>>gvs[]>>
                    simp[Once evaluate_def,panPropsTheory.eval_upd_clock_eq,
                         panPropsTheory.opt_mmap_eval_upd_clock_eq1]>>
                    CASE_TAC>>fs[]
              ...
              strip_tac>>fs[ELIM_UNCURRY]>>

              last_x_assum $ qspec_then ‘SUC k’ assume_tac>>fs[dec_clock_def]>>
              rfs[]>>
...
first_assum $ qspec_then ‘k'’ mp_tac>>
          impl_tac >- cheat>>
          Cases_on ‘evaluate(While e c,s)’>>
          ‘q'=SOME TimeOut ∧ r' = empty_locals s’
            by (fs[Once evaluate_def,panPropsTheory.eval_upd_clock_eq,
                   panPropsTheory.opt_mmap_eval_upd_clock_eq1]>>rfs[])
               fs[empty_locals_defs]>>
          ‘0 < k'’ by (Cases_on ‘k'’>>gvs[])
...
Cases_on ‘n’>>fs[FUNPOW_SUC]>>
      ...
      >- (gvs[]>>
          ‘ X >>= Y ≈ (Ret w >>= Y)’ by
            (irule itree_bind_resp_t_wbisim>>fs[])>>
          fs[Abbr‘X’]>>Cases_on ‘w’>>rename1 ‘Ret (q',r')’>>
          gvs[Abbr‘Y’]>>
          gvs[mrec_sem_simps,ltree_lift_cases]>>
          drule itree_wbisim_Ret_FUNPOW>>strip_tac>>fs[])>>
      fs[Abbr‘X’]>>
      drule_then drule nonret_imp_timeout'>>strip_tac>>fs[]>>
      first_assum $ qspec_then ‘s.clock’ mp_tac>>
      qmatch_goalsub_abbrev_tac ‘evaluate(_,X)’>>
      ‘X=s with locals := s.locals|+(m,x)’
        by simp[Abbr‘X’,state_component_equality]>>fs[]>>
      strip_tac>>fs[]>>gvs[Abbr‘X’]>>
      pop_assum kall_tac>>

      last_x_assum $ qspecl_then [‘s with locals := s.locals |+ (m,x)’] mp_tac>>
      impl_tac >-
       (rpt strip_tac>>fs[]>>
        ...
        simp[Once evaluate_def,panPropsTheory.eval_upd_clock_eq])>>
      strip_tac>>gvs[]>>
      qmatch_asmsub_abbrev_tac ‘X >>= _ = _’>>
  ...
  >- (imp_res_tac strip_tau_FUNPOW>>fs[]>>
      Cases_on ‘t’>>
      fs[strip_tau_simps,strip_tau_FUNPOW_cancel]
      >- (‘∀p. ¬ (ltree_lift query_oracle s.ffi (Ret x') ≈ Ret p)’
            by (irule nonret_trans>>
                irule_at Any ltree_lift_FUNPOW_wbisim>>
                metis_tac[])>>
          fs[ltree_lift_cases]>>
      ...
      conj_tac >-
       (rpt strip_tac>>
        ...
        gvs[Once evaluate_def,panPropsTheory.eval_upd_clock_eq]>>
        pairarg_tac>>fs[]>>
        pairarg_tac>>fs[])>>
      fs[Abbr‘X’]>>
      ...
      Cases_on ‘X’>>fs[]>>
      gvs[mrec_sem_simps,ltree_lift_cases,to_stree_simps]>>
      TRY (fs[Once itree_wbisim_cases]>>NO_TAC)>>
      Cases_on ‘a’>>
      gvs[mrec_sem_simps,ltree_lift_cases,to_stree_simps]>>
...
Cases_on ‘n’>>fs[FUNPOW_SUC]>>
      ...
      first_x_assum $ qspecl_then [‘s with locals := s.locals |+ (m,x')’,‘prog’] assume_tac>>
      gvs[]>>
      ‘∀k'. 0 < k' ⇒
            (SND (evaluate (prog,s with locals := s.locals |+ (m,x)))).ffi.io_events =
            (SND
             (evaluate
              (prog,s with <|locals := s.locals |+ (m,x); clock := k'|>))).ffi.io_events’
        by (rpt strip_tac>>
            last_x_assum $ qspec_then ‘k'’ assume_tac>>fs[]>>
            fs[Once evaluate_def,panPropsTheory.eval_upd_clock_eq]>>gvs[]>>
            pairarg_tac>>fs[]>>
            pairarg_tac>>fs[])>>
      fs[]>>
(*      ‘(s with locals := s.locals |+ (m,x')).ffi = s.ffi’
        by simp[state_component_equality]>>fs[]>>*)
      qmatch_asmsub_abbrev_tac ‘X >>= Y’>>
      Cases_on ‘∃w. X ≈ Ret w’
      >- (fs[Abbr‘X’]>>Cases_on ‘w’>>rename1 ‘Ret (q,r')’>>
      ...
      qmatch_goalsub_abbrev_tac ‘to_stree X >>= _’>>
      Cases_on ‘∃t. strip_tau X t’>>fs[]
  >- (imp_res_tac strip_tau_FUNPOW>>fs[]>>
      Cases_on ‘t’>>
      fs[strip_tau_simps,strip_tau_FUNPOW_cancel]
      >- (‘∀p. ¬ (ltree_lift query_oracle s.ffi (Ret x') ≈ Ret p)’
          ...
          fs[ltree_lift_cases]>>
          fs[Once itree_wbisim_cases])>>
      gvs[to_stree_FUNPOW_eq]>>
 
     rpt (pop_assum mp_tac)>>


      strip_tac>>
      qpat_x_assum ‘∀e k. _ ≠ FUNPOW Tau _ _’ mp_tac>>simp[]>>
      imp_res_tac itree_wbisim_Ret_FUNPOW'>>
      Cases_on ‘∃k' e. X = FUNPOW Tau n' (Vis e k')’>>fs[]
      >- metis_tac[]>>
      ...
      qmatch_goalsub_abbrev_tac ‘to_stree X’>>
      Cases_on ‘X’>>gvs[ltree_lift_cases,to_stree_simps]>>
      
      Cases_on ‘n'’>>fs[FUNPOW_SUC]>>
      dxrule itree_wbisim_Ret_FUNPOW'>>strip_tac>>
      Cases_on ‘to_stree u’>>
      fs[itree_bind_thm]>>
      pairarg_tac>>fs[mrec_sem_simps,to_stree_simps,ltree_lift_cases]>>
      TRY (fs[Once itree_wbisim_cases]>>NO_TAC)>>
metis_tac[]               
...
Theorem stree_trace_LNIL_imp_spin:
  (∀p. ¬(ltree_lift q st ht ≈ Ret p)) ∧
  stree_trace q p st (to_stree ht) = [||] ⇒
  (to_stree ht) = spin
Proof
  strip_tac>>
  fs[stree_trace_def]>>
...
Theorem bounded_trace_equality:
  (∀k'. s.clock < k' ⇒ (SND(evaluate(prog:'a prog,s))).ffi.io_events
                       = (SND(evaluate(prog,s with clock:=k'))).ffi.io_events) ∧
  (∀p. ¬(ltree_lift query_oracle s.ffi (mrec_sem (h_prog (prog,unclock s))) ≈ Ret p)) ∧
 (* s.clock = 0 ∧*) good_dimindex (:'a) ⇒
...
Proof
  ...
  >- (Cases_on ‘op’>>
      ...
      fs[Once itree_wbisim_cases])
  (* Seq *)
  >- (fs[h_prog_def,h_prog_rule_seq_def,
          panPropsTheory.eval_upd_clock_eq,ltree_lift_cases,
         mrec_sem_simps,to_stree_simps,stree_trace_simps]>>
      rpt CASE_TAC>>rpt FULL_CASE_TAC>>
      fs[mrec_sem_simps,to_stree_simps,stree_trace_simps,stree_trace_Vis,ltree_lift_cases,
           msem_lift_monad_law,to_stree_monad_law,ltree_lift_monad_law]>>
      ...
      >-(fs[Abbr‘X’]>>Cases_on ‘p’>>rename1 ‘Ret (q,r')’>>
         ...
         >- (drule_then drule ltree_Ret_to_evaluate'>>strip_tac>>gvs[]>>
             ...
             >- (drule LESS_EQUAL_ADD>>strip_tac>>fs[]>>
                 ...
                 strip_tac>>fs[IS_PREFIX_APPEND]>>
                 fs[LAPPEND_ASSOC]>>
                 last_x_assum irule>>
                 rpt strip_tac>>
                 drule (GSYM LESS_ADD)>>strip_tac>>fs[]>>
                 last_x_assum $ qspec_then ‘k+p+p'’ assume_tac>>
                 fs[]>>rfs[]>>
                 qpat_abbrev_tac ‘X=evaluate _’>>
                 Cases_on ‘X’>>fs[]>>
                 ‘s with clock := k + (p + p')
                  = (s with clock := s.clock + p')’
                   by simp[state_component_equality]>>
                 pop_assum (fn h => fs[Once h])>>
                 qpat_x_assum ‘evaluate (c1,s)=_’ assume_tac>>
                 drule panPropsTheory.evaluate_add_clock_eq>>
                 strip_tac>>fs[])>>
             fs[NOT_LESS_EQUAL]>>
             first_assum $ qspec_then ‘k’ assume_tac>>fs[]>>rfs[]>>
             Cases_on ‘q’>>fs[]>>
             ...
             >- (imp_res_tac (GSYM LESS_ADD)>>fs[]>>
                 ...
                 first_x_assum $ qspec_then ‘p'’ assume_tac>>
                 fs[]>>rfs[]>>
                 last_x_assum $ qspec_then ‘p'+s.clock’ assume_tac>>
                 fs[])>>
             imp_res_tac LESS_ADD_1>>fs[]>>
             drule panPropsTheory.evaluate_add_clock_eq>>
             strip_tac>>fs[]>>rfs[]>>
             Cases_on ‘x = TimeOut’>>fs[]
             >- (
(*
...
(*             first_x_assum $ qspec_then ‘k'’ assume_tac>>fs[]>>
             ...
             qpat_x_assum ‘evalaute (c1,s)=_’ assume_tac>>
             drule panPropsTheory.evaluate_io_events_mono>>
             strip_tac>>fs[IS_PREFIX_APPEND]>>
             rev_drule panPropsTheory.evaluate_io_events_mono>>
             strip_tac>>fs[IS_PREFIX_APPEND]>>

             gvs[]>>
                

             first_x_assum $ qspec_then ‘SUC n + s.clock’ $ assume_tac o GSYM>>
             rfs[]>>
...
(*             Cases_on ‘p’>- fs[]>>*)
             simp[GSYM LAPPEND_fromList]>>
             simp[LAPPEND11_FINITE1,LFINITE_fromList]>>
             rev_drule panPropsTheory.evaluate_io_events_mono>>
             strip_tac>>fs[IS_PREFIX_APPEND]>>fs[]>>
             fs[GSYM LAPPEND_fromList]>>
             fs[LAPPEND11_FINITE1,LFINITE_fromList]>>
             Cases_on ‘evaluate (c2,reclock r' with clock := k')’>>fs[]>>
             drule panPropsTheory.evaluate_io_events_mono>>
             strip_tac>>fs[IS_PREFIX_APPEND]>>
             gvs[]>>
             qpat_x_assum ‘fromList l' = _’ $ assume_tac o GSYM>>
             simp[GSYM LAPPEND_fromList]>>
             simp[LAPPEND11_FINITE1,LFINITE_fromList]>>

             ‘s'.ffi.io_events = r.ffi.io_events’ by gvs[]>>
             fs[]>> (* need r = r' *)
*)
              cheat)>>
             first_x_assum $ qspec_then ‘p+1’ assume_tac>>
             gvs[Once ADD_ASSOC])>>
         fs[Once itree_wbisim_cases])>>
      fs[Abbr‘X’]>>
      fs[Once evaluate_def]>>
  ...
  >- (fs[h_prog_def,h_prog_rule_cond_def,
      ...
      (Cases_on ‘∃p. X ≈ Ret p’
       ...
       strip_tac>>gvs[]))
  (* While *)
  >- (drule_then drule clock_0_imp_LNIL>>fs[]>>strip_tac>>
      fs[Once mrec_sem_while_unfold,
         panPropsTheory.eval_upd_clock_eq,ltree_lift_cases,
         mrec_sem_simps,to_stree_simps,stree_trace_simps]>>
      rpt CASE_TAC>>
      ...
      >-(fs[Abbr‘X’]>>Cases_on ‘p’>>rename1 ‘Ret (q,r')’>>
         ...
         last_x_assum mp_tac>>
         simp[Once evaluate_def]>>
         simp[SimpR“$=”,Once evaluate_def]>>strip_tac>>
         simp[SimpR“$=”,Once evaluate_def]>>
         CASE_TAC>>fs[]
         >- fs[empty_locals_defs,LAPPEND_NIL_2ND]>>
         fs[panPropsTheory.eval_upd_clock_eq]>>gvs[]>>
         Cases_on ‘evaluate (c,dec_clock s)’>>gvs[]>>
         Cases_on ‘q' ≠ SOME TimeOut ∧ q' ≠ SOME Error’
         >- (
         ‘∃m. evaluate (c,dec_clock s) = (q, reclock r' with clock := m)’
           by (Cases_on ‘k ≤ (dec_clock s).clock’>>fs[]
               >- (dxrule LESS_EQUAL_ADD>>strip_tac>>fs[]>>
                   rev_drule panPropsTheory.evaluate_add_clock_eq>>
                   strip_tac>>fs[]>>
                   first_x_assum $ qspec_then ‘p’ assume_tac>>gvs[]>>
                   ‘s with clock := k + p = dec_clock s’
                     by fs[dec_clock_def]>>fs[]>>metis_tac[])>>
               fs[NOT_LESS_EQUAL]>>
               drule_then drule ltree_lift_corres_evaluate>>
               strip_tac>>gvs[]>>
               ‘(dec_clock s).ffi = s.ffi’ by simp[dec_clock_def]>>
               fs[]>>
               drule_then rev_drule wbisim_Ret_unique>>
               strip_tac>>gvs[]>>
               qexists ‘r.clock’>>simp[state_component_equality])>>
         gvs[]>>
         rpt (CASE_TAC>>fs[])>>
         fs[Abbr‘Y’]>>
         gvs[mrec_sem_simps,ltree_lift_cases]>>
         ...
         >- (irule EQ_TRANS>>
             ...
             conj_tac >-
              (rpt strip_tac>>
               drule (GSYM LESS_ADD)>>strip_tac>>fs[]>>
               first_assum $ qspec_then ‘s.clock+p’ assume_tac>>fs[]>>
               rfs[]>>
               rev_drule panPropsTheory.evaluate_add_clock_eq>>
               strip_tac>>fs[]>>
               first_x_assum $ qspec_then ‘p’ assume_tac>>
               fs[dec_clock_def]>>rfs[]>>fs[])>>
             simp[panPropsTheory.eval_upd_clock_eq]>>
             fs[Once mrec_sem_while_unfold])>>
         (* q = SOME Continue *)
         irule EQ_TRANS>>
         ...
         simp[panPropsTheory.eval_upd_clock_eq]>>
         fs[Once mrec_sem_while_unfold])>>
         (* q = SOME TimeOut ∨ q = SOME Error *)
         fs[Abbr‘Y’]>>
         gvs[mrec_sem_simps,ltree_lift_cases]>>
         reverse (Cases_on ‘(dec_clock s).clock < k’)>>fs[NOT_LESS]
         (* q = SOME TimeOut *)
         >- (drule LESS_EQUAL_ADD>>strip_tac>>fs[]>>
             rev_drule panPropsTheory.evaluate_add_clock_eq>>
             strip_tac>>fs[]>>
             ...
             fs[dec_clock_def]>>rfs[]>>fs[])
         >- (drule (GSYM LESS_ADD)>>strip_tac>>fs[]>>
             first_assum $ qspec_then ‘s.clock + p’ mp_tac>>
             impl_tac >- rfs[]>>strip_tac>> fs[]>>
             ‘s with clock := p + (dec_clock s).clock
              = dec_clock (s with clock := p + s.clock)’
             by simp[dec_clock_def]>>fs[]>>
             rpt CASE_TAC>>
             gvs[mrec_sem_simps,ltree_lift_cases]>>
             TRY (fs[Once itree_wbisim_cases]>>NO_TAC)>>
             ...
             simp[LAPPEND11_FINITE1,LFINITE_fromList]>>
             ‘(dec_clock s).ffi = s.ffi’ by simp[dec_clock_def]>>fs[]>>
             qspecl_then [‘c’,‘s with clock := s.clock-1’,‘p’] assume_tac(panPropsTheory.evaluate_add_clock_io_events_mono)>>
             fs[IS_PREFIX_APPEND]>>rfs[dec_clock_def]>>fs[]>>
             fs[GSYM LAPPEND_fromList]>>
             qpat_x_assum ‘r.ffi.io_events = _’ $ assume_tac o GSYM>>
             fs[]>>
             qpat_x_assum ‘_ = r.ffi.io_events’ $ assume_tac o GSYM>>
             fs[]>>
             qpat_x_assum ‘s.ffi.io_events ++ _ = _’ $ assume_tac o GSYM>>
             fs[]>>
             fs[GSYM LAPPEND_fromList]>>
             fs[LAPPEND_ASSOC]>>
             fs[LAPPEND11_FINITE1,LFINITE_fromList]>>
             qpat_x_assum ‘LAPPEND _ _ = _’ $ assume_tac o GSYM>>
             fs[]>>
             cheat)
         (* q = SOME Error *)
         ...
         strip_tac>>fs[]>>
         drule (GSYM LESS_ADD)>>strip_tac>>fs[]>>
         first_x_assum $ qspec_then ‘p’ assume_tac>>gvs[dec_clock_def]>>
         ‘(dec_clock s).ffi = s.ffi’ by simp[dec_clock_def]>>
         fs[]>>
         drule_then rev_drule wbisim_Ret_unique>>
         strip_tac>>fs[]>>
      ...
      Cases_on ‘s.clock’>>fs[]
      >- simp[Once evaluate_def,LAPPEND_NIL_2ND,empty_locals_defs]>>
  ...
  >- (drule_then drule clock_0_imp_LNIL>>fs[]>>strip_tac>>
      ...
      >- (fs[Abbr‘X’]>>Cases_on ‘p’>>rename1 ‘Ret (q',r')’>>
          ...
          >- fs[empty_locals_defs,LAPPEND_NIL_2ND]>>
          TOP_CASE_TAC>>fs[]>>
          rename1 ‘evaluate (q,_) = (q2,r2)’>>
          Cases_on ‘q2 ≠ SOME TimeOut ∧ q2 ≠ SOME Error’
          >- (drule_then drule ltree_lift_corres_evaluate>>
              strip_tac>>gvs[]>>
              ‘(dec_clock s).ffi = s.ffi’ by simp[dec_clock_def]>>
              fs[]>>
              drule_then rev_drule wbisim_Ret_unique>>
              strip_tac>>gvs[]>>
              rpt (CASE_TAC>>fs[h_handle_call_ret_def])>>
              gvs[mrec_sem_simps,ltree_lift_cases]>>
              TRY (fs[Once itree_wbisim_cases]>>NO_TAC)
              >- (gvs[h_handle_call_ret_def,set_var_defs]>>
                  imp_res_tac stree_trace_ret_events'>>gvs[]>>
                  imp_res_tac (INST_TYPE [delta|->alpha] stree_trace_bind_append)>>
                  gvs[]>>
                  gvs[h_handle_call_ret_def,set_var_defs]>>
                  gvs[mrec_sem_simps,ltree_lift_cases,to_stree_simps,stree_trace_simps]>>
                  simp[GSYM LAPPEND_ASSOC]>>
                  last_x_assum irule>>
                  rpt strip_tac>>
                  drule (GSYM LESS_ADD)>>strip_tac>>fs[]>>
                  last_x_assum $ qspec_then ‘s.clock + p’ assume_tac>>fs[]>>rfs[]>>
                  fs[Once evaluate_def,
                     panPropsTheory.eval_upd_clock_eq,
                     panPropsTheory.opt_mmap_eval_upd_clock_eq1]>>
                  ...
                  first_x_assum $ qspec_then ‘p’ assume_tac>>
                  fs[dec_clock_def]>>
                  ‘1≤s.clock’ by simp[]>>fs[GSYM LESS_EQ_ADD_SUB]>>
                  gvs[set_var_defs])>>
              gvs[mrec_sem_simps,ltree_lift_cases,to_stree_simps,stree_trace_simps]>>
              FULL_CASE_TAC>>fs[]>>
              gvs[mrec_sem_simps,ltree_lift_cases,to_stree_simps,stree_trace_simps]>>
              fs[Once itree_wbisim_cases])>>
          (* q = SOME TimeOut ∨ q = SOME Error *)
          gvs[mrec_sem_simps,ltree_lift_cases,empty_locals_defs]>>
          imp_res_tac stree_trace_ret_events'>>gvs[]>>
          imp_res_tac (INST_TYPE [delta|->alpha] stree_trace_bind_append)
          >- (gvs[]>>cheat)>>
          drule_then drule ltree_lift_corres_evaluate_error>>
...
(*
Theorem evaluate_finite_LFINITE:
  ∀prog s.
  good_dimindex (:'a) ∧
  (∀p. ¬(ltree_lift query_oracle s.ffi (mrec_sem (h_prog (prog,unclock s))) ≈ Ret p)) ∧
  (∃k'. ∀k. k' ≤ k ⇒ LENGTH (SND (evaluate (prog:'a prog,s with clock := k))).ffi.io_events = LENGTH (SND (evaluate (prog:'a prog,s with clock := k'))).ffi.io_events) ⇒
  LFINITE (stree_trace query_oracle event_filter s.ffi (to_stree (mrec_sem (h_prog (prog,unclock s))))) (*∧
  LENGTH (stree_trace query_oracle event_filter s.ffi (to_stree (mrec_sem (h_prog (prog,unclock s))))) =
  LENGTH (LUB
   {fromList
    (SND (evaluate (prog,s with clock := k))).ffi.io_events | k | T})*)
Proof
...
Proof
  ...
  qabbrev_tac ‘f = (λx. LENGTH (SND (evaluate (prog, reclock s with clock := x))).ffi.io_events)’>>fs[]>>
  ‘∀k k'. k ≤ k' ⇒ f k ≤ f k'’
    by (fs[Abbr‘f’]>>
        rpt strip_tac>>
        drule LESS_EQUAL_ADD>>strip_tac>>fs[]>>
        assume_tac (Q.SPECL [‘prog:'a prog’,‘reclock s with clock := k’,‘p’] panPropsTheory.evaluate_add_clock_io_events_mono)>>
        fs[IS_PREFIX_APPEND])>>
  ‘∃k. ∀k'. k ≤ k' ⇒ f k' ≤  f k’ by
...
Proof
  ...
  >- (CONV_TAC SYM_CONV >> fs [] >>
      ...
      >- (rw [FORALL_PROD] >>fs[GSYM FORALL_PROD]>>
          ...
          fs[PULL_EXISTS]>>
          drule not_less_opt_lemma>>strip_tac>>gvs[]>>
          qabbrev_tac ‘x=reclock s with clock := k'’>>
          ‘∀k. x.clock < k ⇒
               (SND (evaluate (prog,x))).ffi.io_events =
